...
DataIngestionModule.factory('RuleGroupFactory', function ($http, $location) {
	...
	factory.executeRuleGroup = function (uuid, version) {
		...
		return $http({
			method: 'POST',
			url: url + "ingest/executeGroup?action=execute&uuid=" + uuid + "&version=" + version,
		}).
			then(function (response, status, headers) {
				return response;
			})
...
DataIngestionModule.service("RuleGroupService", function ($q, RuleGroupFactory, sortFactory) {
