...
public class DQOperator {
	...
	public String generateSql(DataQual dataQual, List<String> datapodList, DataQualExec dataQualExec, DagExec dagExec, Set<MetaIdentifier> usedRefKeySet, Mode runMode) throws Exception {
		Datapod srcDP = null;
		...
		if (dataQual.getDependsOn().getRef().getType() == MetaType.datapod) {
			...
			dataQual.getAttribute().setAttrName(srcDP.getAttribute(Integer.parseInt(dataQual.getAttribute().getAttrId())).getName());
			MetaIdentifier srcDPRef = new MetaIdentifier(MetaType.datapod, srcDP.getUuid(), srcDP.getVersion());
			usedRefKeySet.add(srcDPRef);
			return generateSql(dataQual, 
					getTableName(srcDP, datapodList, dagExec, runMode), 
				...
				return generateSql(dataQual, 
						getTableName(srcDP, datapodList, dagExec, runMode), 
						null, datapodList, dataQualExec, dagExec, usedRefKeySet, runMode);
			}
		}
		if (dataQual.getDependsOn().getRef().getType() == MetaType.dataset) {
			/*dataset = (Dataset) daoRegister.getRefObject(dataQual.getAttribute().getRef());
			String attr = datasetServiceImpl.getAttributeSql(daoRegister, dataset, Integer.parseInt(dataQual.getAttribute().getAttributeId()));
			if (attr == null) {
				return null;
			}
			return generateSql(dataQual, 
	...
	public String getTableName(Datapod datapod, List<String> datapodList, DagExec dagExec, Mode runMode) throws Exception {
	...
	private String generateSelect(DataQual dq, DataQualExec dataQualExec, String tableName, String attributeName) throws JsonProcessingException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, NullPointerException, ParseException {
	...
	//	String select = SELECT.concat("row_number() over (partition by 1) as rownum, ")
		String select = SELECT
				 .concat(tildeSepAttrs(datapod.getName(),getRowKeyList(datapod)))
				 .concat(AS)
				 .concat(ROWKEY)
				 .concat(COMMA)
				 .concat(SINGLE_QUOTE)
				 .concat(dq.getDependsOn().getRef().getUuid())
				 .concat(SINGLE_QUOTE)
				 .concat(AS)
				 .concat(DATAPODUUID)
				 .concat(COMMA)
				 .concat(SINGLE_QUOTE)
				 .concat(dq.getDependsOn().getRef().getVersion())
				 .concat(SINGLE_QUOTE)
				 .concat(AS)
				 .concat(DATAPODVERSION)
				 .concat(COMMA)
				 .concat(SINGLE_QUOTE)
				 .concat(datapod.getName())
				 .concat(SINGLE_QUOTE)
				 .concat(AS)
				 .concat(DATAPOD_NAME)				 				 
				 .concat(COMMA)
		...
		if (dq.getAttribute() != null) {
			select = select
					.concat(dq.getAttribute().getAttrId());
		}
		select = select
				 .concat(SINGLE_QUOTE)
				 .concat(AS)
		...
		if (dq.getAttribute() != null) {
			select = select
					.concat(" CAST(")
					.concat(datapod.getName()).concat(DOT).concat(attributeName)
					.concat(dataSource.getType().equalsIgnoreCase(ExecContext.MYSQL.toString()) ? " AS CHAR(50)) " : (dataSource.getType().equalsIgnoreCase(ExecContext.ORACLE.toString()) ? " AS VARCHAR2(70)) " : " AS STRING)"));
		} else {
			select = select
					.concat(SINGLE_QUOTE)
					.concat(SINGLE_QUOTE);
		...
		select = select
				 .concat(AS)
				 .concat(ATTRIBUTE_VAL)
				 .concat(COMMA)
				 .concat(generateCase(dq, tableName, attributeName))
				 .concat(COMMA);
		select = select
				.concat(dataQualExec.getVersion())
				.concat(AS)
				.concat(VERSION);
		return select;
	}
	
	private String generateFrom(DataQual dq, MetaIdentifier ref, List<String> datapodList, DagExec dagExec, Set<MetaIdentifier> usedRefKeySet, Mode runMode) throws Exception {
		Datapod srcDP = null;
		String resp = null;
		...
		/*if (ref.getType() == MetaType.relation) {
			relation = (Relation) daoRegister.getRefAS CHAR(50))Object(ref);
			return FROM.concat(relationOperator.generateSql(relation, null, null, null)).concat(WHERE_1_1);
		}
		if (ref.getType() == MetaType.dataset) {
			dataSet = (Dataset) daoRegister.getRefObAS CHAR(50))ject(ref);
			Map innerMap = (Map) daoRegister.getRefObject(dataSet.getDependsOn().getRef());
			return FROM.concat("(").concat(mapOperator.generateSql(innerMap, null, null, null)).concat(") as ").concat(dataSet.getName()).concat(WHERE_1_1);
		}*/
		return resp
				.concat(generateRefIntFrom(dq, getTableName(srcDP, datapodList, dagExec, runMode), srcDP.getName(), datapodList, dagExec, usedRefKeySet, runMode))
				//.concat(generateStddevFrom(dq, getDataQualTableName(srcDP, datapodList, dagExec), srcDP.getName(), datapodList, dagExec))
				.concat(generateDupCheckFrom(dq, getTableName(srcDP, datapodList, dagExec, runMode), srcDP.getName(), usedRefKeySet));
	}
	
	/*private String generateStddevFrom (DataQual dq, String tableName, String attributeName, List<String> datapodList, DagExec dagExec) {
		String stddevFromStr = null;
		if (dq == null || StringUtils.isBlank(dq.getStdDevCheck())) {
			return EMPTY;
		}
		Datapod datapod = (Datapod) daoRegister.getRefObject(dq.getDependsOn().getRef());
		stddevFromStr = 
			...
			.concat(BRACKET_OPEN)
			.concat(SELECT)
			.concat(commaSepAttrs(getRowKeyList(datapod)))
			.concat(COMMA)
			.concat("STDDEV_POP(")
			.concat(dq.getAttribute().getAttrName())
			.concat(BRACKET_CLOSE)
			.concat(AS)
			.concat(" stddev_pop_alias ")
			.concat(FROM)
			.concat(getDataQualTableName(datapod, datapodList, dagExec))
			.concat(AS)
			.concat(datapod.getName())
			.concat(GROUP_BY)
			.concat(commaSepAttrsGroup(getRowKeyList(datapod)))
			.concat(BRACKET_CLOSE)
			.concat(AS)
			.concat(" stddev_alias ")
			.concat(ON)
		...
		stddevFromStr = stddevFromStr.substring(0, stddevFromStr.length() - AND.length())
							.concat(BRACKET_CLOSE);
		return stddevFromStr;
	}*/
	...
	private String generateDupCheckFrom (DataQual dq, String tableName, String attributeName, Set<MetaIdentifier> usedRefKeySet) throws JsonProcessingException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, NullPointerException, ParseException {
	       if (dq == null || StringUtils.isBlank(dq.getDuplicateKeyCheck()) || dq.getDuplicateKeyCheck().equalsIgnoreCase("N")) {
	           return EMPTY;
	       }
	       Datapod datapod = (Datapod) daoRegister.getRefObject(dq.getDependsOn().getRef());
		   usedRefKeySet.add(dq.getDependsOn().getRef());
	       boolean isKey = false;
	       String dupJoinStr = LEFT_OUTER_JOIN
	                               .concat(BRACKET_OPEN)
	                               .concat(SELECT);
	       if (dq.getAttribute() != null) {
	       isKey = getIsKey(datapod, dq.getAttribute().getAttrName());
	       if (!isKey) {
	           dupJoinStr= dupJoinStr
	.concat(dq.getAttribute().getAttrName())
	                           .concat(AS)
	.concat(dq.getAttribute().getAttrName())
	                           .concat(COMMA);
	...
	.concat(commaSepAttrsGroup(getRowKeyList(datapod)))
	       ...
	       dupJoinStr = dupJoinStr.substring(0, dupJoinStr.length() - AND.length())
	                               .concat(BRACKET_CLOSE);
	       } else {
	           dupJoinStr = dupJoinStr
	                   .concat(commaSepAttrs(getRowKeyList(datapod)))
	...
	private String tildeSepAttrs (String tableName,List<Attribute> attrList) throws JsonProcessingException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, NullPointerException, ParseException {
	...
	private String generateRefIntFrom (DataQual dq, String tableName, String attributeName, List<String> datapodList, DagExec dagExec, Set<MetaIdentifier> usedRefKeySet, Mode runMode) throws Exception {
		...
		refIntStr = LEFT_OUTER_JOIN
					.concat(getTableName(datapodRef, datapodList, dagExec, runMode))
					//.concat(AS)
					.concat(" ")
					.concat(datapodRef.getName())
					.concat("_ref")
					.concat(ON)
					.concat(BRACKET_OPEN);
		...
		refIntStr = refIntStr.substring(0, refIntStr.length() - AND.length())
				.concat(BRACKET_CLOSE);
		return refIntStr;
	}
	...
	public String generateFilter(List<AttributeRefHolder> filterInfo, Set<MetaIdentifier> usedRefKeySet) throws JsonProcessingException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, NullPointerException, ParseException {
	...
	public String generateSql(DataQual dq, String tableName, String attributeName, List<String> datapodList, DataQualExec dataQualExec, DagExec dagExec, Set<MetaIdentifier> usedRefKeySet, Mode runMode) throws Exception {
		...
		return select
				.concat(generateFrom(dq, dq.getDependsOn().getRef(), datapodList, dagExec, usedRefKeySet, runMode))
				.concat(WHERE_1_1).concat(generateFilter(dq.getFilterInfo(), usedRefKeySet));
	}
	
	
	public String generateCase(DataQual dq, String tableName, String attributeName) throws JsonProcessingException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, NullPointerException, ParseException {
		Datasource dataSource = commonServiceImpl.getDatasourceByApp();
		...
		if (dq.getRangeCheck() != null && !dq.getRangeCheck().isEmpty()) {
			boolean containsLB = dq.getRangeCheck().containsKey("lowerBound") && (dq.getRangeCheck().get("lowerBound") != null);
			boolean containsUB = dq.getRangeCheck().containsKey("upperBound") && (dq.getRangeCheck().get("upperBound") != null);
			...
			if (containsLB && containsUB) {
				check = tableAttr
						.concat(BETWEEN).concat("'").concat(lowerBound).concat("'")
						.concat(AND).concat("'").concat(upperBound).concat("'").concat(BLANK);
				dqBuilder.append(caseWrapper(check, colName)).append(COMMA);
			} else if (containsLB) {
		...
		if (StringUtils.isNotBlank(dq.getDataTypeCheck())) {
			check = " CAST(".concat(tableAttr).concat(" AS ").concat(dq.getDataTypeCheck()).concat(") IS NOT NULL").concat(BLANK);
		...
		if (StringUtils.isNotBlank(dq.getDateFormatCheck())) {
			check = " DATE_FORMAT(".concat(tableAttr).concat(",'").concat(dq.getDateFormatCheck()).concat("') IS NOT NULL").concat(BLANK);
		...
		if (dq.getLengthCheck() != null && !dq.getLengthCheck().isEmpty()) {
			boolean containsLB = dq.getLengthCheck().containsKey("minLength") && (dq.getLengthCheck().get("minLength") != null);
			boolean containsUB = dq.getLengthCheck().containsKey("maxLength") && (dq.getLengthCheck().get("maxLength") != null);
			Long lowerBound = dq.getLengthCheck().get("minLength");
			Long upperBound = dq.getLengthCheck().get("maxLength");
			colName = LENGTH_CHECK_PASS;
			if (containsLB && containsUB) {
				check = " LENGTH(CAST(".concat(tableAttr).concat(dataSource.getType().equalsIgnoreCase(ExecContext.MYSQL.toString()) ? " AS CHAR(50)) " : (dataSource.getType().equalsIgnoreCase(ExecContext.ORACLE.toString()) ? " AS VARCHAR2(70)) " : " AS STRING)")).concat(BETWEEN).concat(lowerBound+"").concat(AND).concat(upperBound+"").concat(BLANK);
				dqBuilder.append(caseWrapper(check, colName)).append(COMMA);
			} else if (containsLB) {
				check = " LENGTH(CAST(".concat(tableAttr).concat(dataSource.getType().equalsIgnoreCase(ExecContext.MYSQL.toString()) ? " AS CHAR(50)) " : (dataSource.getType().equalsIgnoreCase(ExecContext.ORACLE.toString()) ? " AS VARCHAR2(70)) " : " AS STRING)")).concat(GREATER_THAN).concat(lowerBound+"").concat(BLANK);
				dqBuilder.append(caseWrapper(check, colName)).append(COMMA);
			} else if (containsUB) {
				check = " LENGTH(CAST(".concat(tableAttr).concat(dataSource.getType().equalsIgnoreCase(ExecContext.MYSQL.toString()) ? " AS CHAR(50)) " : (dataSource.getType().equalsIgnoreCase(ExecContext.ORACLE.toString()) ? " AS VARCHAR2(70)) " : " AS STRING)")).concat(LESS_THAN).concat(lowerBound+"").concat(BLANK);
				dqBuilder.append(caseWrapper(check, colName)).append(COMMA);
			} else {
		...
		if (dq.getRefIntegrityCheck() != null 
				&& dq.getRefIntegrityCheck().getRef() != null) {
			Datapod refIntTab = (Datapod) daoRegister.getRefObject(dq.getRefIntegrityCheck().getRef());
			dq.getRefIntegrityCheck().setAttrName(refIntTab.getAttribute(Integer.parseInt(dq.getRefIntegrityCheck().getAttrId())).getName());
			check = refIntTab.getName()
						.concat("_ref")
						.concat(DOT)
						.concat(dq.getRefIntegrityCheck().getAttrName())
						.concat(IS_NOT_NULL);
		...
		/*if (StringUtils.isNotBlank(dq.getStdDevCheck())) {
			check = " stddev_pop_alias ".concat(GREATER_THAN).concat(dq.getStdDevCheck()).concat(BLANK);
			colName = STDDEV_CHECK_PASS;
			dqBuilder.append(caseWrapper(check, colName)).append(COMMA);
		} else {
	...
	private String caseWrapper(String check, String colName) {
		StringBuilder caseBuilder = new StringBuilder(CASE_WHEN)
									.append(check)
									.append(THEN)
									.append(colName)
									.append(BLANK);
