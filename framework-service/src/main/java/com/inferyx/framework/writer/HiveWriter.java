...
public class HiveWriter implements IWriter {
	...
	public void write(Dataset<Row> df, String filePathUrl, Datapod d, String saveMode) throws IOException {
		try {
			...
			// daoRegister.getiDatasourceDao().findOneByUuidAndVersion(d.getDatasource().getRef().getUuid(),d.getDatasource().getRef().getVersion());
			...
			if (parList.size() > 0) {
				...
				df.sqlContext().sql("set hive.exec.dynamic.partition.mode="
				...
				df.write().mode(SaveMode.Append).partitionBy(columnList)
						.insertInto(dataSource.getDbname().concat(".").concat(d.getName()));
			} else {
				if (saveMode.equalsIgnoreCase("append"))
					df.write().mode(SaveMode.Append)
							.saveAsTable(dataSource.getDbname().concat(".").concat(d.getName()));
