...
AdminModule.service('AdminActivityService', function ($q, AdminActivityFactory, sortFactory) {
	...
	this.getLatestByUuid = function (uuid, type) {
		...
		AdminActivityFactory.findLatestByUuid(uuid, type).then(function (response){onSuccess(response.data)},function (response){onError(response.data)});
	...
	this.getByOneUuidandVersion = function (uuid, version, type) {
		var deferred = $q.defer();
		AdminActivityFactory.findByOneUuidandVersion(uuid, version, type)
