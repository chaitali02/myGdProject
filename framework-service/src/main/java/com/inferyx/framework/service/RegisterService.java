...
*/
	...
	public List<AttributeRefHolder> getAttributesByRelation(String relationUuid, String type)
		...
		if (type.equalsIgnoreCase("relation")) {
			...
			// relationServiceImpl.findLatestByUuid(relationUuid);
			...
			// MetaType sourceType = relationDetails.getDependsOn().getRef().getType();

			// objectUuidList.add(sourceUUId);
			...
			/*
			 * if (objectUuidList.size() > 0) {
			 * 
			 * for (String uuid : objectUuidList) { if (sourceType.equals(MetaType.datapod))
			 * { Datapod datapodDet = (Datapod) commonServiceImpl.getLatestByUuid(uuid,
			 * MetaType.datapod.toString()); List<Attribute> listAttributes =
			 * datapodDet.getAttributes();
			 * 
			 * MetaIdentifier ref = new MetaIdentifier(); ref.setType(MetaType.datapod);
			 * ref.setUuid(datapodDet.getUuid()); ref.setVersion(datapodDet.getVersion());
			 * ref.setName(datapodDet.getName()); for (Attribute attr : listAttributes) {
			 * 
			 * AttributeRefHolder attributeRef = new AttributeRefHolder();
			 * attributeRef.setAttrId(Integer.toString((attr.getAttributeId())));
			 * attributeRef.setAttrType(datapodDet.getAttribute(attr.getAttributeId()).
			 * getType()); if (datapodDet.getAttribute(attr.getAttributeId()).getDispName()
			 * != null) {
			 * attributeRef.setAttrName(datapodDet.getAttribute(attr.getAttributeId()).
			 * getDispName()); } else
			 * attributeRef.setAttrName(datapodDet.getAttribute(attr.getAttributeId()).
			 * getName()); attributeRef.setRef(ref); attrRefDetails.add(attributeRef); } }
			 * if (sourceType.equals(MetaType.dataset)) { DataSet datasetDet = (DataSet)
			 ...
			 * 
			 * MetaIdentifier ref = new MetaIdentifier(); ref.setType(MetaType.dataset);
			 * ref.setUuid(datasetDet.getUuid()); ref.setVersion(datasetDet.getVersion());
			 * ref.setName(datasetDet.getName()); for (AttributeSource attr :
			 * listAttributes) {
			 * 
			 * AttributeRefHolder attributeRef = new AttributeRefHolder();
			 * attributeRef.setAttrId((attr.getAttrSourceId()));
			 * attributeRef.setAttrType(attr.getSourceAttr().getAttrType());
			 * attributeRef.setAttrName(attr.getAttrSourceName()); attributeRef.setRef(ref);
				...
				for (int i = 0; i < relationDetails.getRelationInfo().size(); i++) {
					for (int j = 0; j < relationDetails.getRelationInfo().get(i).getJoinKey().size(); j++) {
						for (int k = 0; k < relationDetails.getRelationInfo().get(i).getJoinKey().get(j).getOperand()
							...
							// Datapod datapodDet = datapodServiceImpl.findLatestByUuid(datapodUUID);
							...
							if (refType.equalsIgnoreCase(MetaType.datapod.toString())) {
								...
								if (attr != null) {
									...
									attributeRef.setAttrName(attr.getName());
