...
DatascienceModule.factory('VartifAnalysisFactory', function ($http, $location) {
    var factory = {};
    ...
    factory.findPrediction = function (uuid, version, type,data) {
        var url = $location.absUrl().split("app")[0]
        return $http({
            url: url + "model/getPrediction?action=view&uuid=" + uuid +"&version="+version +"&type=" + type,
            method: "POST",
...
DatascienceModule.service("VartifAnalysisService", function ($http, VartifAnalysisFactory, $q, sortFactory,$filter) {
    this.getPrediction = function (uuid, version, type, data) {
        var deferred = $q.defer();
        VartifAnalysisFactory.findPrediction(uuid, version, type, data).then(function (response) { onSuccess(response.data)},function (response) { onError(response.data) });
    ...
    this.deploy = function (uuid, version, type) {
        var deferred = $q.defer();
        VartifAnalysisFactory.deploy(uuid, version, type).then(function (response) { onSuccess(response.data)},function (response) { onError(response.data) });
        var onSuccess = function (response) {
            deferred.resolve({
                data: response
            });
    ...
    this.undeploy = function (uuid, version, type) {
        var deferred = $q.defer();
        VartifAnalysisFactory.undeploy(uuid, version, type).then(function (response) { onSuccess(response.data)},function (response) { onError(response.data) });
        var onSuccess = function (response) {
            deferred.resolve({
                data: response
            });
    ...
    this.getTrainExecViewByCriteria = function (uuid, version, type, trainexecuuid,startdate, enddate,active, status) {
        var deferred = $q.defer();
        VartifAnalysisFactory.findTrainExecViewByCriteria(uuid, version, type, trainexecuuid,startdate, enddate,active, status).then(function (response) { onSuccess(response.data) });
        var onSuccess = function (response) {
            var result=[];
            ...
            if(response && response.length >0){
                ...
                resultInfo.createdOn=response[i].createdOn;
    ...
    this.getTrainExecByModel = function (uuid, version, type) {
        var deferred = $q.defer();
        VartifAnalysisFactory.findTrainExecByModel(uuid, version, type).then(function (response) { onSuccess(response.data) });
        var onSuccess = function (response) {
            deferred.resolve({
                data: response
            });
    ...
    this.getAllVersionByUuid = function (uuid, type) {
        var deferred = $q.defer();
        VartifAnalysisFactory.findAllVersionByUuid(uuid, type).then(function (response) { onSuccess(response.data) });
        var onSuccess = function (response) {
            deferred.resolve({
                data: response
            });
    ...
    this.getAllLatest = function (type) {
        var deferred = $q.defer();
        VartifAnalysisFactory.findAllLatest(type).then(function (response) { onSuccess(response.data) });
        var onSuccess = function (response) {
            deferred.resolve({
                data: response
            });
