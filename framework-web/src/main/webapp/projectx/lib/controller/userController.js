(function() {
	...
	'use strict';
	var userModule = angular.module('user_model', [ 'datatables' ]);

	userModule.controller('user_controller', user_controller);

	function user_controller(DTOptionsBuilder, DTColumnBuilder, $scope,$rootScope) {

		$rootScope.$on("CallParentMethod", function() {
			$scope.changeData();
		});

		$scope.dtOptions = DTOptionsBuilder.fromSource("/framework/metadata/getAll?type=user")
	...
	userModule.controller('LHS_controller', function($scope, $rootScope) {

		$scope.selectUser = function(userdata) {
			$rootScope.menuvalue = userdata;
