...
public class RunRuleServiceImpl extends RunBaseRuleService {
	...
	protected String getTableName(BaseRule baseRule, BaseRuleExec baseRuleExec, MetaIdentifier datapodKey,
		...
		Datapod dp = (Datapod) commonServiceImpl.getOneByUuidAndVersion(datapodKey.getUuid(), datapodKey.getVersion(),
		...
		if (datasource.getType()
				.equals(ExecContext.FILE.toString())
				|| runMode.equals(RunMode.ONLINE)) {
			return String.format("%s_%s_%s", datapodKey.getUuid().replace("-", "_"), datapodKey.getVersion(),
					baseRuleExec.getVersion());
		}
