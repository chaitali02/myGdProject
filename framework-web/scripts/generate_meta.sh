...
do
	...
	name=`mongo --quiet --host $hostname $dbname --eval "printjson(db.$collection.find({"_id":$obj},{name:1,uuid:1}).toArray())" | grep "name" | awk -F":" '{print $2}' | sed 's/^[ \t]*//;s/[ \t]*$//' | tr ' ' '_' | tr [A-Z] [a-z] | sed 's/_-_/_/g' | xargs`
	if [[ $name != "" ]] ; then
