...
public class ModelController {
	...
	public @ResponseBody String executeScript(@RequestParam("scriptName") String scriptName,
									...
									@RequestParam(value = "modelExecVersion", required = false) String modelExecVersion) throws IOException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, NullPointerException, ParseException {
	...
	public com.inferyx.framework.domain.Algorithm getAlgorithmByModelExec(@RequestParam("modelExecUUID") String modelExecUUID,
	...
	public HttpServletResponse  download(@RequestParam(value= "modelExecUUID") String ruleExecUUID, 
	    		...
	    		{
			Mode runMode = Helper.getExecutionMode(mode);
			HttpServletResponse result=modelServiceImpl.download(ruleExecUUID, ruleExecVersion,response, runMode);
	    	return null;
	}
	
	@RequestMapping(value = "/predict/execute", method = RequestMethod.POST)
	public boolean predict(@RequestParam("uuid") String predictUUID, 
		@RequestParam("version")String predictVersion,
	    @RequestBody (required = false) ExecParams execParams,
		@RequestParam(value = "type", required = false) String type,
		@RequestParam(value = "action", required = false) String action, 
		...
		try {
			...
			if(paramMapList.size()>0){
				for(ParamMap paramMap : paramMapList){
					predictExec = modelServiceImpl.create(predictUUID, predictVersion, execParams, paramMap, predictExec);
					modelServiceImpl.predict(predictUUID, predictVersion, execParams, type, action, mode);	
				} 
			}else {
				predictExec = modelServiceImpl.create(predictUUID, predictVersion, execParams,null, predictExec);
				modelServiceImpl.predict(predictUUID, predictVersion, execParams, type, action, mode);	
			}
		}catch (Exception e) {
			e.printStackTrace();
	...
	public boolean simulate(@RequestParam("uuid") String simulateUUID, 
		...
		try {
			...
			if(paramMapList.size()>0){
				for(ParamMap paramMap : paramMapList){
					simulateExec = modelServiceImpl.create(simulateUUID, simulateVersion, execParams, paramMap, simulateExec);
					modelServiceImpl.simulate(simulateUUID, simulateVersion, execParams, type, action, mode);	
				} 
			}else {
				simulateExec = modelServiceImpl.create(simulateUUID, simulateVersion, execParams,null, simulateExec);
				modelServiceImpl.simulate(simulateUUID, simulateVersion, execParams, type, action, mode);	
			}
		}catch (Exception e) {
			e.printStackTrace();
	...
	public List<Model> getAllModelByType(@RequestParam(defaultValue = "SPARK") String modelType, 
			@RequestParam(value = "action", required = false) String action) throws Exception {	
		return modelExecServiceImpl.getAllModelByType(modelType, action);
	}
}