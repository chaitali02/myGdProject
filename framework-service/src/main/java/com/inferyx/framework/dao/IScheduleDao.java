...
public interface IScheduleDao extends MongoRepository<Schedule, String> {
	...
	public List<Schedule> findAllByUuid(String appUuid, String uuid);

	@Query(value = "{ 'appInfo':{$elemMatch: { 'ref.uuid': ?0}},'uuid' : ?1 }")
	public Schedule findLatestByUuid(String appUuid, String uuid, Sort sort);
	...
	@Query(value = "{ 'uuid' : ?ScheduleScheduleSchedule0, 'version':{$lte:?1 }}")
