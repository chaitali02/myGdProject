...
export class ExpressionComponent implements OnInit {
  ...
  constructor(private _location: Location, config: AppConfig, private activatedRoute: ActivatedRoute, public router: Router,private appHelper: AppHelper, private _commonService: CommonService) {
    this.uuid = '';
  ...
  onSuccessgetOneByUuidAndVersion(response: Expression) {
    ...
    this.expression.published =this.appHelper.convertStringToBoolean(response.published) ;
    this.expression.active = this.appHelper.convertStringToBoolean(response.active);
    this.expression.locked = this.appHelper.convertStringToBoolean(response.locked);
  ...
  getAllAttributeBySource(defaultValue, index, type) {debugger
  ...
  OnSuccesgetAllAttributeBySource(response2, defaultValue, index, type) {
    ...
    for (const n in response2) {
      ...
      allname1.value.uuid = response2[n].uuid;
      allname1.value.id = response2[n].id;
      allname1.value.datapodname = response2[n].datapodname;
      allname1.value.name = response2[n].name;
      allname1.value.dname = response2[n].dname;
      allname1.value.attributeId = response2[n].attributeId;
      temp[n] = allname1;
    }
    this.allAttribute = temp;
    if (defaultValue == true && index != null) {

      let lhsdatapodAttribute = null
      if (type == 'lhs') {
  ...
  selectlhsType(type, index) {
    if (type == "string") {
      ...
      this.expressionTableArray[index].lhsvalue = null;
  ...
  selectrhsType(type, index) {
    ...
    if (type == "string") {
      ...
      this.expressionTableArray[index].rhsvalue = null;
    }
    else if (type == "datapod") {
      ...
      this.expressionTableArray[index].isrhsFormula = false;
      this.expressionTableArray[index].sourceattribute = null
  ...
  addRow() {
    ...
    if (this.expressionTableArray == null || this.expressionTableArray.length == 0) {
      this.expressionTableArray = [];
      expessioninfo.logicalOperator = '';
    }

    else {
      expessioninfo.logicalOperator = this.logicalOperators[1];
    }
  ...
  submitExpression() {
    ...
    expressionjson.locked = this.expression.locked == true ? 'Y' : "N"
    ...
    var met = new AttributeRefHolder();
    var metref = new MetaIdentifier();
    ...
    var notMet = new AttributeRefHolder();
    var notMetref = new MetaIdentifier();
    ...
    if (this.expressionTableArray.length > 0) {
      for (var i = 0; i < this.expressionTableArray.length; i++) {
        var expressioninfo = new FilterInfo();
        var operand = []
        var lhsoperand = new SourceAttr()
        var lhsref = new MetaIdentifier();
        var rhsoperand = new SourceAttr()
        var rhsref = new MetaIdentifier();
