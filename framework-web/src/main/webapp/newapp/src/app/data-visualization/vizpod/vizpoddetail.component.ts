...
export class VizpodDetailComponent {
    ...
    // OnSuccesgetAllAttributeBySource12(response){ 
    //     let attribute2=[]
    //     for (const n in response) {
    //         let allname={};
    //         allname["id"]=response[n]['uuid']+"_"+response[n]['attributeId'];
    ...
    //         allname["attrId"]=response[n]['attributeId'];
    //         allname["attrName"]=response[n]['name'];
    //         attribute2[n]=allname
    //     }
    //     this.allAttributes=attribute2;
    //     this.getAllFormula();
    //     this.getAllExpression();
    //     this.dropdownList = attribute2;
    // }
    ...
    onSuccessGetOneByUuidAndVersion(response){
       ...
       this.vizpoddata.detailAttr=response.detailAttr.ref.name + "." + response.detailAttr.attributeName
   ...
   removeKey(index){
    ...
    submit(){
        ...
        if (this.filterAttributeTags != null) {
        	for (let i = 0; i < this.filterAttributeTags.length; i++) {
        		let attributeInfo = {}
        		let ref = {};
        		ref['type'] = "datapod";
