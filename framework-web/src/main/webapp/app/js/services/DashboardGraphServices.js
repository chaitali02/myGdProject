...
DatavisualizationModule.factory('DahsboardFactory',function($http,$location){
    ...
    factory.findAttributeValues=function(uuid,attributeId) {
...
DatavisualizationModule.service('DahsboardSerivce',function($q,sortFactory,DahsboardFactory){
	...
	this.getAttributeValues=function(uuid,attributeId){
		   var deferred=$q.defer();
		   DahsboardFactory.findAttributeValues(uuid,attributeId).then(function(response){onSuccess(response.data)});
	...
	this.getGraphData=function(uuid,version,degree){
	   ...
	   this.getLatestByUuid=function(id,type){
		   ...
		   var onSuccess=function(response){
			 ...
			 /* dashboardjson.dashboarddata.filterInfo=[];
			  var vizpodarray=[];
			  for(var i=0;i<response.sectionInfo.length;i++){
                    vizpodarray[i]=response.sectionInfo[i].vizpodInfo;
			  }
			  dashboardjson.vizpod=vizpodarray
			  console.log(JSON.stringify(dashboardjson.vizpod))
			    deferred.resolve({
		                  data:dashboardjson
		              })*/
