...
DataQualityModule.service("DataqulityService", function ($q, DataQualityFactory, sortFactory) {
	this.generateDq = function (type, uuid, version, data) {
		...
		DataQualityFactory.generateDq(type, uuid, version, data).then(function (response) { onSuccess(response.data) }, function (response) { onError(response.data) });
	...
	this.genIntelligence = function (type, uuid, version) {
		var deferred = $q.defer();
		DataQualityFactory.genIntelligence(type, uuid, version).then(function (response) { onSuccess(response.data) }, function (response) { onError(response.data) });
		var onSuccess = function (response) {
			deferred.resolve({
				data: response
			});
