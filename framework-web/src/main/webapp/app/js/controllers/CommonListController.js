  ...
  CommonModule.controller('CommonListController', function($location,$http,cacheService,dagMetaDataService, uiGridConstants, $state, $stateParams, $rootScope, $scope, $sessionStorage, CommonService, FileSaver, Blob, $filter,cacheService, privilegeSvc, $timeout) {
    ...
    $scope.updateStats();
    var groups = ['profileexec', 'profilegroupexec', 'dqexec', 'dqgroupexec', 'ruleexec', 'rulegroupexec'];
    // $scope.pagination={
    //   currentPage:1,
    //   pageSize:10,
    ...
    $scope.nonExecTypes = ['datapod','dataset','expression','filter','formula','function','load','relation','algorithm','paramlist','paramset','activity','application','datasource','datastore','group','privilege','role','session','user','vizpod'];
    ...
    $scope.ok = function() {
      ...
      if ($scope.select == 'rule' || $scope.select == 'model') {
        $scope.getExecParams();
      } else {
    ...
    $scope.uploadFile=function(){
      ...
      CommonService.uploadFile($scope.uploaaduuid,fd,"datapod").then(function(response){onSuccess(response.data)});
			var onSuccess=function(response){
