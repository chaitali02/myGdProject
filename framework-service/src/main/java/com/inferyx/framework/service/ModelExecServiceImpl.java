...
//		iDataQualExecDao.delete(ID);		
	...
	public List<ModelExec> getModelExecByModel(String modelUuid, String modelVersion) {
		...
		List<ModelExec> execList = mongoTemplate.find(query, ModelExec.class);
		return execList;
	}
	public Algorithm getAlgorithmByModelExec(String modelExecUUID, String modelExecVersion) throws JsonProcessingException {
		ModelExec modelExec = (ModelExec) commonServiceImpl.getOneByUuidAndVersion(modelExecUUID, modelExecVersion,
				MetaType.modelExec.toString());
		Model model = (Model) commonServiceImpl.getOneByUuidAndVersion(modelExec.getDependsOn().getRef().getUuid(),
				modelExec.getDependsOn().getRef().getVersion(), MetaType.model.toString());
	...
	public List<Model> getAllModelByType(String modelType, String action) {
		Query query = new Query();
		query.fields().include("uuid");
		query.fields().include("version");
		query.fields().include("name");
		...
		query.addCriteria(Criteria.where("type").is(modelType));
