...
public class DataSourceFactory {
	...
	@Autowired private HiveRegister hiveRegister;
	@Autowired private CSVRegister csvRegister;
	@Autowired private HiveReader hiveReader;
	@Autowired private ParquetReader parquetReader;
	@Autowired private HiveWriter hiveWriter;
	@Autowired private ParquetWriter parquetWriter;
	@Autowired private ImpalaRegister impalaRegister;
	@Autowired private ImpalaWriter impalaWriter;
	@Autowired private ImpalaReader impalaReader;
	@Autowired private OracleWriter oracleWriter;
	@Autowired private OracleReader oracleReader;
	@Autowired private MySqlReader mySqlReader;
	@Autowired private MySqlWriter mySqlWriter;
	...
	public IReader getDatapodReader(Datapod dp, MetadataUtil commonActivity) throws JsonProcessingException {
		...
		case "IMPALA": return impalaReader;
		case "ORACLE": return oracleReader;
		case "MYSQL": return mySqlReader;
		//case "spark": return (dp.getName().equalsIgnoreCase("hive")? hiveReader : parquetReader);
		default:
		}
		return null;
	}	
	public IWriter getDatapodWriter(Datapod datapod, MetadataUtil daoRegister) throws JsonProcessingException {
		...
		Datasource datasource = (Datasource) daoRegister.getRefObject(new MetaIdentifier(MetaType.datasource,datapod.getDatasource().getRef().getUuid(),datapod.getDatasource().getRef().getVersion()));
		...
		case "IMPALA": impalaWriter.setDaoRegister(daoRegister);
			return impalaWriter;
		case "MYSQL" : mySqlWriter.setDaoRegister(daoRegister);
			return mySqlWriter;
		case "ORACLE": oracleWriter.setDaoRegister(daoRegister);
			return oracleWriter;
		default:
		}
		return null;
