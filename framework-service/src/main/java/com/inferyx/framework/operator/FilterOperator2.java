...
public class FilterOperator2 {
	...
	public String generateSql(List<AttributeRefHolder> filterIdentifierList
			...
			, Boolean isAggrReqd, RunMode runMode, Datasource mapSourceDS
			, Map<String, String> paramValMap) throws Exception {
		return generateSql(filterIdentifierList, refKeyMap, otherParams, usedRefKeySet, null, isAggrAllowed, isAggrReqd, runMode, mapSourceDS, paramValMap);
	}
	...
	public String generateSql(List<AttributeRefHolder> filterIdentifierList
			...
			, Boolean isAggrAllowed
			, Boolean isAggrReqd, RunMode runMode
		...
		for (int i = 0; i < filterIdentifierList.size(); i++) {
			...
			case filter : 
				...
				String filterStr = joinKeyOperator.generateSql(filter.getFilterInfo(), filter.getDependsOn(), refKeyMap, otherParams, usedRefKeySet, execParams,isAggrAllowed, isAggrReqd, runMode, mapSourceDS, paramValMap);
	...
	public String generateSql(List<FilterInfo> filterInfo
		...
		String filterStr = joinKeyOperator.generateSql(filterInfo, filterSource, refKeyMap, otherParams, usedRefKeySet, execParams,isAggrAllowed, isAggrReqd, runMode, mapSourceDS, attributeList, paramValMap);
		if (StringUtils.isBlank(filterStr)) {
			builder.append(ConstantsUtil.BLANK);
		} else {
			builder.append(" AND (").append(filterStr).append(")");
	...
	public String generateSelectWithFilter(List<AttributeRefHolder> filterIdentifierList, Set<MetaIdentifier> usedRefKeySet,
											ExecParams execParams, Datasource mapSourceDS, Map<String, String> paramValMap) throws Exception {
		StringBuilder builder = new StringBuilder();
		...
		for (AttributeRefHolder filterIdentifier : filterIdentifierList) {
			...
			switch (filterIdentifier.getRef().getType()) {
				...
				case filter : 
					...
					builder.append(" (").append(joinKeyOperator.generateSql(filter.getFilterInfo(),filter.getDependsOn(), null, null, usedRefKeySet, execParams, true, false, null, mapSourceDS, paramValMap)).append(")");
