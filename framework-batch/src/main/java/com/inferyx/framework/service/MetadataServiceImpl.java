...
//		metadata.exportBaseProperty();
	...
	public List<BaseEntity> getBaseEntityByCriteria(String type, String name, String userName, String startDate, String endDate, String tags, String active, String uuid, String version, String published) throws ParseException, JsonProcessingException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, NullPointerException {
		...
		try {
			...
			if (userName != null && !userName.isEmpty()) {
				...
				if(user != null && user.getUuid().equals(getCurrentUser().getUuid())) {
					...
					//logger.info("else part...1.0");
					if(user != null)
						query.addCriteria(Criteria.where("_id").ne("1").andOperator(Criteria.where("createdBy.ref.uuid").in(user.getUuid()),Criteria.where("published").in("Y")));
				}
			}else {					
				//logger.info("else part...1.1");
				User currentUser = getCurrentUser();
				if(currentUser != null)
					query.addCriteria(Criteria.where("_id").ne("1").orOperator(Criteria.where("createdBy.ref.uuid").in(currentUser.getUuid()),Criteria.where("published").in("Y")));
			}
		...
		try {
			...
			if (userName != null && !userName.isEmpty()) {
				...
				if(user != null && user.getUuid().equals(getCurrentUser().getUuid())) {
				  ...
				  if(user != null)
				   criteriaList.add(where("_id").ne("1").andOperator(where("createdBy.ref.uuid").is(user.getUuid()),where("published").is("Y")));
				}
			}
//			else{					
//				User currentUser = getCurrentUser();
...
//			}
			...
			if ( (startDate != null && !startDate.isEmpty()) && (endDate != null && !endDate.isEmpty()) ){
				criteriaList.add(where("_id").ne("1").and("createdOn").lte(simpleDateFormat.parse(endDate)).gte(simpleDateFormat.parse(startDate)));
				}
