...
DatascienceModule.controller('CreateModelController', function($state, $stateParams, $rootScope, $scope, $sessionStorage, $timeout, $filter, ModelService,$http,$location) {
  ...
  $scope.changeCheckboxExecution = function() {
    ...
    if ($scope.checkboxModelexecution == "YES" && !$scope.checkboxCustom) {
      ModelService.getParamSetByAlgorithm($scope.selectalgorithm.uuid, $scope.selectalgorithm.version).then(function(response) {
        onSuccessGetParamSetByAlgorithm(response.data)
      });
  ...
  $scope.onChangeAlgorithm = function() {
    ...
    $scope.allparamset = null;
    if ($scope.selectalgorithm.labelRequired == "Y") {
  ...
  $scope.submitModel = function() {
    ...
    if ($scope.isLabelDisable == false && $scope.selectLabel!=null) {
      var label = {};
      var ref = {};
...
DatascienceModule.controller('ResultModelController', function($filter, dagMetaDataService, $rootScope, $scope, ModelService) {
  ...
  $scope.getExec = function(data) {
    ...
    ModelService.getModelResult(uuid, version).then(function(response) {
      onSuccessGetModelResult(response.data)
    });
