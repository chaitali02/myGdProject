...
public class CloneDataOperator implements Operator {
	...
	public String execute(com.inferyx.framework.domain.Operator operator, ExecParams execParams,
		...
		if (otherParams.containsKey("datapodUuid_" + ((Datapod)sourceObj).getUuid() + "_tableName")) {
			sourceSql = otherParams.get("datapodUuid_" + ((Datapod)sourceObj).getUuid() + "_tableName");
